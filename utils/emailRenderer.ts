import { EmailBlock, EmailDocument } from '../types';

export const generateEmailHTML = (doc: EmailDocument): string => {
    ...
    const toHtml = (b: EmailBlock): string => {
        switch (b.type) {
            ...
            case 'footer': return `
                    <div style="background:${b.backgroundColor};padding:32px 24px;text-align:center;font-family:${settings.fontFamily};">
                        ...
                        ${b.socialLinks.length > 0 ? `
                            <div style="margin-bottom:24px;">
                                ${b.socialLinks.map(s => {
                const sizeMap: Record<string, string> = { small: '24px', medium: '32px', large: '40px' };
                const radMap: Record<string, string> = { circle: '50%', square: '0', rounded: '8px' };
            ...
            // Add other new blocks if needed
            default: return '';
        }
    };

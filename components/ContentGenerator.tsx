...
import { PenTool, Facebook, Instagram, Linkedin, AtSign, Video, Search, CheckSquare, Square, Sparkles, Copy, Check, Loader2, History, Trash2, Clock, X, Edit3, ShieldCheck } from 'lucide-react';
...
const ContentGenerator: React.FC<ContentGeneratorProps> = ({ initialData }) => {
  ...
  useEffect(() => {
    if (showHistory) {
        setHistory(StorageService.getContentHistory());
    }
  }, [showHistory]);
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        StorageService.addContentHistory(newRecord);

    } catch (e) {
        alert("Có lỗi xảy ra khi tạo content. Vui lòng thử lại.");
    } finally {
  ...
  const handleDeleteHistory = (e: React.MouseEvent, id: string) => {
      ...
      if(confirm("Xóa lịch sử này?")) {
          StorageService.deleteContentHistory(id);
        ...
        <div className="flex gap-3">
             ...
             <div className="flex items-center bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                <button 
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${!useManualMode ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    <ShieldCheck size={14}/> Brand Vault
                </button>
                <button 
                    onClick={() => setUseManualMode(true)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${useManualMode ? 'bg-slate-100 text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
                >
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl shadow-sm border border-slate-200 hover:bg-slate-50 transition-colors text-slate-600 font-medium"
            >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-2xl border transition-all text-left
                      ${isSelected 
                        ...
                        : 'border-slate-100 bg-slate-50 text-slate-600 hover:bg-white hover:border-slate-200 hover:shadow-sm'
                      }`}
                  >
                    <platform.icon size={20} strokeWidth={1.5} className={isSelected ? 'text-white' : platform.color} />

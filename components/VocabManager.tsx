...
import { Plus, Trash2, Edit2, Upload, FileText, FolderPlus, Save, X } from 'lucide-react';
...
const VocabManager: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedSetId) {
      setWords(StorageService.getWordsBySet([selectedSetId]));
    } else {
      setWords([]);
    }
  }, [selectedSetId]);
  ...
  const refreshData = () => {
    const loadedSets = StorageService.getSets();
    setSets(loadedSets);
  ...
  const handleAddManual = () => {
    ...
    const newWord: Word = {
      id: Date.now().toString(),
      term: manualEn.trim(),
      definition: manualVi.trim(),
      starred: false,
      setId: selectedSetId
    };
    StorageService.addWords([newWord]);
  ...
  const handleBulkPaste = () => {
    ...
    if (newWords.length > 0) {
      StorageService.addWords(newWords);
